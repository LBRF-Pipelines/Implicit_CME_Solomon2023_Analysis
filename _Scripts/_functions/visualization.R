...
get_learned_trial <- function(dat, p_num, t_num, n_num) {
  subset(dat, id == p_num & trial == t_num & num==n_num)
}
...
# Renders PDFs of recall trials in a summary data frame to a given output path,
# colour coding the sample points using the 'color_code' argument

plot_learned_trials <- function(trials, samples, color_code = NULL, outdir = ".") {
  ...
  for (i in 1:nrow(trials)) {
    vals <- trials[i, ]
    fname <- paste0(paste( #altered as we only have trial and the recall num attempt
      paste0("p", vals$id), paste0("t", vals$trial),
      paste0("r", vals$num),
      sep = "_"
    ), ".pdf")
    tmp <- get_learned_trial(samples, vals$id, vals$trial, vals$num)
    if (is.null(color_code)) {
      plt <- plot_figure_pts(tmp$x, tmp$y, path = TRUE)
    } else {
      tmp$col <- with(tmp, eval(parse(text = color_code)))
